...
| `_lookup.py` | `unwrap_or_raise(obj, exc)` — the shared single-row-lookup helper backing the public `get`/`get_or_none` pair across all five namespaces (ADR-0019 Enforcement tier-2). Every namespace `get()` calls it directly to raise its `*NotFoundError` on a miss (the v0.8.0 flip, issue #1247). |
