...
| Mind Map | `generate mind-map` | `--interactive` for the studio-artifact kind (³) *(default: note-backed JSON, sync)* | .json |
...
³ **Two kinds of mind map (issue #1256).** `generate mind-map` (default) creates the **note-backed** kind — a JSON node tree, generated synchronously. `generate mind-map --interactive` creates the newer **interactive** studio artifact (what the web app now makes); it is polled to completion. Both list under `artifact list --type mind-map` and export via `download mind-map`. `--instructions` applies only to the note-backed kind (ignored with a warning for `--interactive`).
