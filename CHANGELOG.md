...
This release closes a CLI UX overhaul: a top-to-bottom pass that makes the CLI shell-, CI-, and agent-friendly. The headline changes are uniform `--json` envelopes on every mutating and detail command, an explicit codebase-wide exit-code policy (with two breaking corrections), graceful long-running waits with progress and SIGINT-resume, stdin / env-var / completion conventions across the surface, and the eradication of help-text drift and Python-traceback leaks.
