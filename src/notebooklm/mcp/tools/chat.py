...
def register(mcp: Any) -> None:
    ...
    async def chat_ask(
        ...
        Returns the ``answer``, server-derived ``turn_number``, and citation
        ``references`` (when a question is asked). The internal ``raw_response``
        debugging blob is never included.
