"""Sharing binding rows (P9.3 codec rows; remaining P9.4 custom rows).

``SHARING_GET`` and ``LEGACY_SHARE_ARTIFACT`` are ``encode → one native call →
decode`` codec rows. The two remaining mutate-then-readback composites
(``SHARING_SET_VIEW_LEVEL``, ``SHARING_UPDATE_USERS``) are
:class:`CustomBinding` rows: each declares exactly its two natives under the
...
their hoists as P9.2-6/7, after the stop/go review. ``SHARING_SET_PUBLIC`` is
service-owned since P9.2-5. The rows are
