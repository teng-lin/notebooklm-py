...
SEMANTIC_DEADLINE_AUTHORITIES: Final[MappingProxyType[Operation, SemanticDeadlineAuthority]] = (
    MappingProxyType(
        {
            # No operation claims CLIENT_TIMEOUT any more. P10 R3.2-R3.5 took the
            # ``source.add_*`` operations out of this ledger and R5.1a-b the
            # eleven Studio generation families, ``mind_map.generate_note`` last:
            # their rows are single-native now, and the owning service captures
            # the one budget the default-source read and the kickoff share. The
            # authority stays because the ledger is the gate a *new* multi-native
            # row must pass, and this is the seed such a row would declare.
