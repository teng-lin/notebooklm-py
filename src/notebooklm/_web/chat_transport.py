"""Web chat-stream error mapping over the generic runtime transport.
...
``ChatError`` or ``NetworkError`` so the semantic backend binding stays free
of HTTP-status branching. ``WebRpcBackend._chat_ask`` is its production
caller and supplies the shared :class:`RuntimeTransport` directly.
"""
...
async def chat_aware_authed_post(
    ...
    *,
    ...
    the backend binding free of HTTP-status branching.
