...
* The host has no back-reference to :class:`NotebookLMClient` — the client
  reaches it through ``self._web_runtime.composed.chain_host``, but not the other way
...
host (``core._web_runtime.composed.chain_host._<attr> = ...``); there are no client-side
...
class MiddlewareChainHost:
    ...
    async def _authed_post_chain_terminal(self, request: RpcRequest) -> RpcResponse:
        ...
        (``core._web_runtime.composed.chain_host._authed_post_chain_terminal = fake_terminal``)
    ...
    async def await_refresh(self, expected_epoch: int) -> None:
        ...
        """
        await self._auth_refresh.await_refresh(expected_epoch)
