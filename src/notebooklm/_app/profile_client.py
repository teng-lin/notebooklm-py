...
class ProfileClientOwner:
    ...
    async def open(
        ...
        async def own_client() -> bool | None:
            cm = factory()
            ...
            try:
                ...
                if not await cm.__aexit__(type(exc), exc, exc.__traceback__):
                    raise
                return True
            # The lifespan runs in another task. Forward its exception exactly
            # so client cleanup can preserve it and honor deliberate suppression.
            return await cm.__aexit__(*self._exit_info)
        ...
        try:
            waiters: set[asyncio.Future[NotebookLMClient] | asyncio.Task[bool | None]] = {
                ready,
                task,
            }
    ...
    async def close(
        ...
        self._assert_loop()
        self._closed = True
        self._exit_info = (exc_type, exc, tb)
        self._release.set()
