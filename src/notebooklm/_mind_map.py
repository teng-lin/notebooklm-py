"""Note-backed mind maps: the transitional raw note-row service and its facade.
...
:class:`LegacyNoteBackedService` is the deferred raw note-row implementation
those adapters run on. It lives here rather than in :mod:`._note_service` so
the semantic note service stays free of wire imports; it is transitional and
is deleted with the mind-map workflows, together with the private
:class:`NoteRowKind` row classifier.
...
# ``LegacyNoteBackedService`` instances is correct and simpler than
# per-instance bookkeeping — there is no per-instance state on the tasks
# themselves.
