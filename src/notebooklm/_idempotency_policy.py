...
def register_default_policies(registry: IdempotencyRegistry) -> None:
    ...
    #     ``_web.codec.chat_saved_note`` (issue #660), dispatched by the
    #     semantic chat backend binding.
    ...
    # ``_web.codec.artifact_payloads.build_*`` helpers). Every position is structural —
    ...
    # ``_artifacts.py`` and ``_web.codec.artifact_payloads.build_mind_map_params``).
