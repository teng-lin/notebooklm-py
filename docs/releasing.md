...
- [ ] Set up the development environment:
  ```bash
  # Canonical contributor install; `[all]` expands to browser+dev+markdown+mcp+server
...
1. **Dep tree from `uv.lock`.** `uv sync --frozen --extra browser --extra dev --extra markdown` installs the locked dependency tree for those selected extras into `.venv/` — the same canonical contributor extras documented in `docs/installation.md`. Other optional extras (`cookies`, `mcp`, `server`) are covered by their own install/test paths. This produces a deterministic dep tree without any TestPyPI lookups.
