...
| `_web/policy.py` | Exact P4 ledger for all 80 directly supported web operations plus the twelve service-owned workflows: semantic policy, every reachable native method/variant (or, for a stream-only row, its declared streamed verb), reviewed native idempotency, and optional reported divergence. |
