...
| `_label_service.py` | Private P6.4 transport-neutral source-label/collection service over one discriminated neutral record family; since P9.2-2/3 it owns the `label.update` and `collection.update` workflows (one get preflight/readback plus one `label.mutate` per member), and since P9.2-8 it owns `label.create` (`label.list` baseline then `label.allocate` echo reconciliation). Each workflow has one deadline and rebinds leaf failures to the workflow operation. |
...
| `_web/labels.py` | P6.4 source-label/collection web workflow mixin; only `collection.create` plus its baseline/readback helper remain (`label.create` and both update workflows are service-owned; the list/get/delete/generate leaves are `_web/bindings/labels.py` rows). |
...
| `_web/policy.py` | Exact P4 ledger for all 80 directly supported web operations plus the seven service-owned workflows: semantic policy, every reachable native method/variant, reviewed native idempotency, and optional reported divergence. |
| `_web/registry.py` | Closed web disposition registry over all 92 operations: 80 directly supported, seven service-owned, and five explicitly unsupported. |
