...
| `_sharing_service.py` | Private P6.5 transport-neutral sharing service; since P9.2-5 it owns the public-link visibility mutate-then-readback workflow over `SHARING_MUTATE` + `SHARING_GET`, including one workflow deadline and leaf-error rebinding. |
...
| `_web/bindings/sharing.py` | Sharing rows: the P9.3 codec rows `SHARING_GET`, `LEGACY_SHARE_ARTIFACT` (`encode → one native call → decode`, the `NativeCallSpec` as sole method authority) and the two remaining P9.4 *deferred-product* `CustomBinding` rows `SHARING_SET_VIEW_LEVEL`, `SHARING_UPDATE_USERS`; `SHARING_SET_PUBLIC` is service-owned since P9.2-5 and sequences `SHARING_MUTATE` plus `SHARING_GET` above the port. |
...
| `_web/policy.py` | Exact P4 ledger for all 82 directly supported web operations plus the four service-owned workflows: semantic policy, every reachable native method/variant, reviewed native idempotency, and optional reported divergence. |
| `_web/registry.py` | Closed web disposition registry over all 91 operations: 82 directly supported, four service-owned, and five explicitly unsupported. |
