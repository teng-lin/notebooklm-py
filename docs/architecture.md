...
| `_notebook_mutation_service.py` | Private transport-neutral notebook mutation service; owns the P9.2 `NOTEBOOK_UPDATE` patch/readback and `NOTEBOOK_CREATE` snapshot/allocate/reconcile/quota workflows under one deadline. |
...
| `_web/registry.py` | Closed web disposition registry over all 97 operations: 80 directly supported (derived from the row table), twelve service-owned, and five explicitly unsupported. |
