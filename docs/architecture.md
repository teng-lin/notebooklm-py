...
| `_notebook_mutation_service.py` | Private transport-neutral notebook mutation service; validates semantic input and owns the P9.2 `NOTEBOOK_UPDATE` patch/readback workflow under one deadline. |
...
| `_web/policy.py` | Exact P4 ledger for all 80 directly supported web operations plus the ten service-owned workflows: semantic policy, every reachable native method/variant, reviewed native idempotency, and optional reported divergence. |
| `_web/registry.py` | Closed web disposition registry over all 95 operations: 80 directly supported, ten service-owned, and five explicitly unsupported. |
