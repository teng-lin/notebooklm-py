...
│   ├── _uploadwidget.py         # register_upload_widget(mcp, config): OPT-IN in-app MCP-App upload widget (ADR-0027, NOTEBOOKLM_MCP_UPLOAD_WIDGET=1). A ui:// resource (file-picker HTML) + source_add_widget tool that emit the claude.ai render gates (_meta.ui.domain = sha256(public-url), flat ui/resourceUri, profile=mcp-app mime) via FastMCP meta=/app=; the widget POSTs bytes to /files/ul (reuses ADR-0024). Off the default surface unless enabled
