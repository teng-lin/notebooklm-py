...
| `_android/notes.py` | Private B6 note CRUD adapter with exact write/read-back checks, bounded deletion polling, a reusable saved-response creation seam, and a private typed note-backed mind-map read seam. |
| `_android/sharing.py` | Private B6 public-link sharing adapter; collaborator and view-level mutations remain evidence-gated. |
| `_android/mind_maps.py` | Private B7 Android mind-map composition over base-typed artifact/note collaborators. Typed note-backed and aggregate reads use B6's exact-kind projection without touching the raw `NotesAPI.list_mind_maps` Web-row boundary. Note-backed rename/delete, hydrated interactive rename, generation, and tree reads remain evidence-gated; explicit non-hydrating interactive rename/delete compose through artifacts. No public client factory selects it. |
