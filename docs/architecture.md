...
| Notes and mind maps | Backend-neutral `NoteService` invokes typed NOTE_* operations for `NotesAPI` and every note-backed MIND_MAP_* workflow, including the raw row listings `NotesAPI.list_mind_maps` publishes; `MindMapFamilyService` owns the interactive Studio branch. Since P9.3 the plain-note leaves and the four mind-map leaves are `_web/bindings/notes.py` / `_web/bindings/mind_maps.py` codec rows over the mixed note-row codec, and since P9.4b the input-defaulting generate composites were custom rows — both left that shape in P10 R5.1b, when `MindMapFamilyService`, `NoteBackedMindMapFamilyService` and `NoteService` took over their default-scope read and their language default. P10 R4.2 hoisted the rest: `ARTIFACT_LIST`/`ARTIFACT_GET` are the `StudioCatalog` merge of `ARTIFACT_CATALOG` and a supplemental `MIND_MAP_LIST` read, `ARTIFACT_GENERATE_MIND_MAP` is `NoteBackedMindMapFamilyService` sequencing `NOTEBOOK_GET`, `MIND_MAP_GENERATE_NOTE` and the note leaves, and the deferred raw note-row service they reached through a row-scoped caller is deleted. |
...
| `_semantic/records/label.py` | P6.4 neutral source-label/collection records and eleven typed operation definitions, re-exported from `_semantic/records/__init__.py`. |
| `_semantic/records/mind_map.py` | Neutral mind-map records and the six typed `mind_map.*` operation definitions (R1.4), re-exported from `_semantic/records/__init__.py`. |
| `_semantic/records/note.py` | P6.3 neutral plain-note records and five typed operation definitions, re-exported from `_semantic/records/__init__.py`. |
| `_semantic/records/notebook.py` | P2/P6.6 neutral notebook records and nine typed operation definitions, re-exported from `_semantic/records/__init__.py`. |
| `_semantic/records/research.py` | P6.2 neutral Research records and four typed operation definitions, re-exported from `_semantic/records/__init__.py`. |
| `_semantic/records/settings.py` | P6.6 neutral account-settings records and three typed operation definitions, re-exported from `_semantic/records/__init__.py`. |
| `_semantic/records/sharing.py` | P6.5 neutral Sharing records and seven typed operation definitions, including the two P9.2 primitives, re-exported from `_semantic/records/__init__.py` while keeping the shared record module below the size ratchet. |
...
| `_semantic/records/source.py` | Frozen, slotted, protocol-neutral source read/add/content/refresh/Drive/upload records, including the P9.2 `SOURCE_PATCH_TITLE` primitive DTOs and the fourteen typed source operation definitions (R1.4), split from `_semantic/records/__init__.py` to keep the shared record surface below the module-size ratchet. |
...
| `_web/backend.py` | Single web semantic backend and P9.4c shell, owner of the runtime/lifecycle/auth/metrics leaves and the construction-audited row-only binding table. P9.3 leaves and P9.4 composites live in `_web/bindings/*`; P9.2 workflows are service-owned above the port. |
...
| `_web/bindings/__init__.py` | `WEB_BINDING_ROWS` (P9.3/P9.4c): the union of every domain's binding rows, checked for one canonical row per directly supported operation. |
...
| `_web/bindings/mind_maps.py` | P9.3 mind-map leaf codec rows (`MIND_MAP_LIST`, `MIND_MAP_GET`, `MIND_MAP_UPDATE`, `MIND_MAP_DELETE`, the P10 R4.2 `MIND_MAP_GENERATE` primitive and, since P10 R5.1b, both generate members). R5.1b retired the last two *deferred-product* rows in the package and the category with them: `MIND_MAP_GENERATE_INTERACTIVE` and `MIND_MAP_GENERATE_NOTE` take an already-resolved scope, and the default-scope `NOTEBOOK_GET` read belongs to `MindMapFamilyService`, `NoteBackedMindMapFamilyService` and `NoteService` above the port — the module has no custom row left. `MIND_MAP_LIST` carries the one scoped `map_error`: the Studio catalog marks its supplemental read so the raw `httpx` leaf the auth-refresh path re-raises becomes a neutral reason for that caller alone, leaving `client.notes`/`client.mind_maps` observing it unchanged. |
| `_web/bindings/notebooks.py` | Notebook codec rows: P9.3 reads (`NOTEBOOK_LIST`, `NOTEBOOK_GET`, `NOTEBOOK_DELETE`, `NOTEBOOK_REMOVE_RECENT`, `NOTEBOOK_SUMMARIZE`, `NOTEBOOK_DESCRIBE`) plus the P9.2 `NOTEBOOK_PATCH` and guarded `NOTEBOOK_ALLOCATE` primitives; row-level mappings expose only required-readback not-found and create quota-rejection evidence to the service-owned workflows. |
...
| `_web/bindings/primitives.py` | P9.2 primitive codec rows: `LABEL_MUTATE` (input-keyed over the five `UPDATE_LABEL` variants, one call per member), `LABEL_ALLOCATE` (manual `CREATE_LABEL` for either dialect), `SHARING_MUTATE` (one `SHARE_NOTEBOOK` visibility or grant envelope), `SOURCE_PATCH_TITLE` (one `UPDATE_SOURCE` title set-op), and `SHARING_PATCH_VIEW_LEVEL` (one `RENAME_NOTEBOOK` viewer-scope field mask) — the single-native leaves the hoisted workflows sequence above the port; plus the P10 R2.2 `CHAT_STREAM_ANSWER` leaf, whose `NativeCallSpec` selects a `StreamNative` and so declares no wire method at all. |
...
| `_web/bindings/chat.py` | P9.3 chat codec rows: `CHAT_GET_CONVERSATION`, `CHAT_GET_HISTORY`, `CHAT_DELETE_HISTORY`, `CHAT_SAVE_NOTE`, and the input-keyed `CHAT_CONFIGURE` (`GET_NOTEBOOK` read or `RENAME_NOTEBOOK` mutation selected from the action) — `encode → one native call → decode`. P10 R2.2 retired the `CHAT_ASK` custom row: `chat.ask` is service-owned over the `CHAT_STREAM_ANSWER` leaf in `_web/bindings/primitives.py`. |
| `_web/bindings/settings.py` | P9.3 settings/suggestions codec rows: `SETTINGS_GET`, `SETTINGS_GET_LIMITS`, `SETTINGS_SET_LANGUAGE`, `ARTIFACT_SUGGEST_REPORTS`, `NOTEBOOK_SUGGEST_PROMPTS` — `encode → one native call → decode` with the `NativeCallSpec` as the sole method authority. P10 R5.1c retired the *deferred-product* row here: `NOTEBOOK_SUGGEST_PROMPTS` takes an already-resolved source scope, and `SuggestionService` issues the default-scope `NOTEBOOK_GET` read above the port. The walker derives their catalog authorities from these module-level assignments. |
...
| `_web/bindings/sources.py` | P9.3 source codec rows: `SOURCE_LIST`, `SOURCE_GET` (exact-id select inside `decode`), `SOURCE_WAIT` (the one `DeadlineMode.IGNORE` row), `SOURCE_DELETE`, `SOURCE_REFRESH`, `SOURCE_CHECK_FRESHNESS`, `SOURCE_GET_GUIDE`, `SOURCE_GET_FULLTEXT`; and, since P9.4b, the source-add family's last `CustomBinding` row, `SOURCE_ADD_FILE` (*protocol*; P10 R3.2/R3.3/R3.4/R3.5 hoisted `SOURCE_ADD_TEXT`, `SOURCE_ADD_URL`, `SOURCE_ADD_DRIVE` and `SOURCE_ADD_URL_BATCH` into `SourceService`), whose handler sequences its declared `snapshot`/`rename`/`register`/`limits` specs through the row-scoped invoker; `upload_backend()` builds the upload pipeline's callbacks over a `SOURCE_ADD_FILE` invoker, and `_honor_requested_title()` is that row's best-effort post-add rename (#1960) — moved down here from the source domain package so `_web` takes no domain import at all (P10 invariant I2); `SourceService._honor_requested_title` owns the record-based contract above the port. Since P9.2-4 `SOURCE_GET` is also the null-echo hydration leaf for service-owned `SOURCE_UPDATE`. |
| `_web/bindings/studio.py` | P9.3 Studio leaf codec rows plus the P9.2 `ARTIFACT_PATCH_TITLE` and plain `ARTIFACT_CATALOG` primitives consumed by service-owned artifact rename. Existing rows cover export, revise, retry, delete, wait (caller deadline), and input-keyed download. P10 R5.1a retired the eight *deferred-product* generation rows: each takes an already-resolved input record and is an ordinary single-native `CodecBinding` over `CREATE_ARTIFACT`, with the default-source read owned by `_studio/generation.py` above the port. |
| `_web/registry.py` | Closed web disposition registry over all 99 operations: 75 directly supported (derived from the row table), 21 service-owned, and three explicitly unsupported. |
...
| `scripts/_web_policy_intent.py` | Hand-reviewed *expected* half of the P4 parity ledger for all 80 directly supported web operations plus the twelve service-owned workflows: semantic policy, every reachable native method/variant (or, for a stream-only row, its declared streamed verb), reviewed native idempotency, workflow leaf edges, and optional reported divergence. Since P10 R2.5 it lives beside the catalog specs rather than in production, so the audit's expected side stays independent of the rows it is compared against. |
...
| `_source/batch.py` | `SourceUrlBatchItem`, the public-model positional outcome the MCP/REST batch endpoints receive; the workflow behind it (one non-replayed `ADD_SOURCE` write, positional attribution, omitted-row reconciliation, fail-closed transport/duplicate ambiguity policy) is service-owned since P10 R3.5 as `SourceService.add_urls_batch` |
...
│   │   ├── chat.py              # Neutral Chat records/operation definitions (P6.1)
│   │   ├── label.py             # Neutral source-label/collection records/operation definitions (P6.4)
│   │   ├── mind_map.py          # Neutral mind-map records/operation definitions (R1.4)
│   │   ├── note.py              # Neutral plain-note inputs/results/records and typed operation definitions
│   │   ├── notebook.py          # Neutral notebook inputs/results/records and typed operation definitions
│   │   ├── research.py          # Neutral Research records/operation definitions (P6.2)
...
│   ├── registry.py              # Closed web dispositions; direct set derived from the row table
