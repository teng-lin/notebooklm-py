...
| `_web/policy.py` | Exact P4 ledger for all 81 directly supported web operations plus the six service-owned workflows: semantic policy, every reachable native method/variant, reviewed native idempotency, and optional reported divergence. |
| `_web/registry.py` | Closed web disposition registry over all 92 operations: 81 directly supported, six service-owned, and five explicitly unsupported. |
