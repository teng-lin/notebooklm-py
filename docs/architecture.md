...
| `_source_service.py` | Private transport-neutral service for source content, refresh, Drive, upload, and remaining source variants over typed operation definitions and records; since P9.2-4 it owns `source.update` as patch-title then conditional source-get hydration under one workflow deadline, and since P10 R3.2/R3.3/R3.5 the `source.add_text`, `source.add_url` and `source.add_url_batch` workflows (the second as source-list baseline, one `source.register` url allocation, a reconciling probe, and the `source.patch_title` finalise; the third delegating to `_source_batch_service.py`). |
| `_source_batch_service.py` | The service-owned `source.add_url_batch` workflow (P10 R3.5): one non-replayed `source.register` url write, positional attribution of its echo, and the conditional `source.list` of ERROR rows. Reached only through `SourceService.add_urls_batch`; it lives beside the service because three hoisted workflows put that module over the ADR-0008 size budget |
| `_source_add_failures.py` | The two neutral failure-report builders the hoisted source-add workflows share: one `BackendErrorReason.SOURCE_ADD` report from a `SourceAddFailureRecord`, and the leaf's own captured public graph |
...
| `_web/bindings/sources.py` | P9.3 source codec rows: `SOURCE_LIST`, `SOURCE_GET` (exact-id select inside `decode`), `SOURCE_WAIT` (the one `DeadlineMode.IGNORE` row), `SOURCE_DELETE`, `SOURCE_REFRESH`, `SOURCE_CHECK_FRESHNESS`, `SOURCE_GET_GUIDE`, `SOURCE_GET_FULLTEXT`; and, since P9.4b, the remaining source-add family as `CustomBinding` rows (`SOURCE_ADD_DRIVE`, `SOURCE_ADD_FILE`, both *protocol*; P10 R3.2 hoisted `SOURCE_ADD_TEXT`, R3.3 `SOURCE_ADD_URL` and R3.5 `SOURCE_ADD_URL_BATCH` into `SourceService`) whose handlers sequence their declared `snapshot`/`create`/`rename`/`register`/`limits` specs through the row-scoped invoker; `upload_backend()` builds the upload pipeline's callbacks over a `SOURCE_ADD_FILE` invoker. Since P9.2-4 `SOURCE_GET` is also the null-echo hydration leaf for service-owned `SOURCE_UPDATE`. |
...
| `_web/policy.py` | Exact P4 ledger for all 80 directly supported web operations plus the eleven service-owned workflows: semantic policy, every reachable native method/variant, reviewed native idempotency, and optional reported divergence. |
| `_web/registry.py` | Closed web disposition registry over all 97 operations: 79 directly supported, thirteen service-owned, and five explicitly unsupported. |
...
├── _source_add_failures.py      # Neutral failure reports the hoisted source-add workflows share
