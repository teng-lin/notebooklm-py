...
| Notes and mind maps | `NoteService` owns web note-row CRUD/classification through `RpcCaller`. `NoteBackedMindMapService` keeps web row decoding and exact-content note-backed rename in `WebMindMapsAPI`; rename fetches the raw stored row and resends its content unchanged. The neutral `MindMapsAPI` composes unified list/lookup/rename/delete workflows over base-typed `ArtifactsAPI` and `NotesAPI` collaborators, using one narrow protected hook for note-backed rename. `AndroidMindMapsAPI` adds no protobuf fields: typed aggregate reads compose through B6's evidenced projection without fabricating Web rows, explicit note-backed delete composes through B6's kind-safe Notes delete, and explicit non-hydrating interactive rename/delete compose through `ArtifactsAPI`; note-backed rename and auto-detected mutation remain evidence-gated. |
...
```text
  ...
  metrics | drain_tracker | call_supervisor | reqid | auth_coord | kernel
  | cookie_persistence | web_transport | root lifecycle
        |
        v
...
| `Kernel` | [`_web/transport/kernel.py`](../src/notebooklm/_web/transport/kernel.py) | Pure web transport core. Owns the epoch-fenced `httpx.AsyncClient` and cookie jar; exposes `post()`, cookie accessors, and `aclose()`. `WebTransportLifecycle`, not the root lifecycle, opens and closes it. Concrete class behind the `Kernel` Protocol in `_web/contracts.py`; constructed by `build_collaborators(...)` and called from the middleware leaf via `RuntimeTransport.terminal → Kernel.post`. |
| `_runtime/init` | [`_runtime/init.py`](../src/notebooklm/_runtime/init.py) | Construction-time helpers for `NotebookLMClient`: validation, collaborator construction (including the shared `CallSupervisor`), web transport/chain wiring, and composition binding. |
...
| `_android/artifacts.py` | Private B4 partial artifact adapter: aggregate Studio/note-backed listing, Studio-only polling, generation dispatch, delete/rename, infographic PNG download, and report suggestions; unresolved mutation/download/export families reject before I/O. |
| `_android/artifact_creation.py` | Exact Android `CreateArtifact` request builders for video, cinematic video, tailored reports/study guides, flashcards, infographics, and slide decks. |
...
| `_android/mind_maps.py` | Private B7 Android mind-map composition over base-typed artifact/note collaborators. Note-backed rename/delete/tree reads and auto-detected mutation compose through evidenced note/artifact operations; unresolved generation and interactive-tree decoding remain evidence-gated. No public client factory selects it. |
...
| `_android/proto/google/internal/labs/tailwind/orchestration/v1/orchestration_service_pb2.py` | Sole exact-package cumulative orchestration descriptor: 40 implemented methods, including seven conventional-name signatures explicitly tracked as current-web-bundle inferences; the separate sharing descriptor adds two exact paths and the exception manifest is empty. |
...
├── _android/                    # Private Android backend package (not publicly selected)
...
│           │   ├── orchestration_service_pb2_grpc.py # 40-method generated stub
