...
| `_label_service.py` | Private P6.4 transport-neutral source-label/collection service over one discriminated neutral record family; since P9.2-2 it owns the `label.update` workflow (one `label.get` preflight/readback plus one `label.mutate` per member under one workflow deadline, leaf failures rebound to `label.update`). |
...
| `_web/labels.py` | P6.4 source-label/collection web workflow mixin; since P9.3 only the create composites and `collection.update` plus the shared set read they preflight through remain here (the list/get/delete/generate leaves are `_web/bindings/labels.py` rows; `label.update` is service-owned since P9.2-2). |
