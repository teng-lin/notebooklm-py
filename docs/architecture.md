...
│   ├── _smoke.py                # Shared deployed-server smoke driver: current Studio artifact selection plus signed upload/download over a real HTTP socket; imported by the release helper and E2E suites so tool renames cannot leave an ungated duplicate
