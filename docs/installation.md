...
| `android` | exact pinned `grpcio` + `protobuf`, plus `gpsoauth>=1.1.0` | Android backend runtime dependencies. Install before selecting `backend="android"` / `--backend android`. Selection applies only to substitution-qualified namespaces; inspect `client.backends` for the installed mix. Excluded from `all`. | `pip install "notebooklm-py[android]"` | `uv add "notebooklm-py[android]"` |
