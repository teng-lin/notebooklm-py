...
| `all` | Resolves to `browser` + `dev` + `headless` + `markdown` + `mcp` + `server` (**not the `android`, `cookies`, or `impersonate` extra**) | Contributors using the established web stack. Because it includes `dev`, it still installs the pinned protobuf compiler/runtime used to verify checked-in stubs; it does not select an Android backend. | `pip install "notebooklm-py[all]"` | `uv add "notebooklm-py[all]"` (in your project) — see [All vs All-Extras](#all-vs-all-extras) |
...
Routes own Pydantic input, HTTP status, and JSON/error projection. They construct the same frozen
generation request variants and use the same per-kind validation table as CLI and MCP; transport-
neutral cores do not contain HTTP or presentation policy. The batch source route validates local
members, sends all valid URLs once, and returns one ordered result per input with `commit_state`
`confirmed`, `rejected`, `unknown`, or `not_sent`. Continuation follows that typed evidence rather
than the HTTP status. Inputs and titles are capped and credential-redacted before public and JSON
projection.
