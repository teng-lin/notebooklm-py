...
| `android` | exact pinned `grpcio` + `protobuf`, plus `gpsoauth>=1.1.0` | Private Android backend development/runtime dependencies. B1 exposes no public backend selector; install only for direct adapter development and tests. Excluded from `all`. | `pip install "notebooklm-py[android]"` | `uv add "notebooklm-py[android]"` |
