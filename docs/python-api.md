...
| `list_play_books()` | - | `list[PlayBook]` | List the account's Google Play Books eligible to add as sources ("Expert Intelligence"; US only, 18+). Empty for an account with no Play Books library. **Web backend only** — the Android backend raises `UnsupportedOperationError`. See [Google Play Books sources](#google-play-books-sources). |
