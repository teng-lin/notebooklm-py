...
| **Artifacts** | `artifact_list(notebook)` · `artifact_generate(notebook, artifact_type, …)` · `artifact_status(notebook, task_id)` · `artifact_get_prompt(notebook, artifact)` (the free-text prompt an artifact was generated from; `null` if none) · `artifact_download(notebook, artifact? | artifact_type?, path?, output_format?, artifact_id?)` (target by `artifact` name-or-id ref **or** by `artifact_type` [+ `artifact_id` for a specific one, else latest]) · `artifact_rename(notebook, artifact, new_title)` · `artifact_retry(notebook, artifact)` (re-run a failed artifact in place; task_id == artifact_id) · `artifact_delete(notebook, artifact, confirm)` |
