...
| R1-04 stable-set commit/loss | Web `client.notebooks.rename(notebook_id, "Stable")` | The first `RENAME_NOTEBOOK` request commits the title then disconnects before response headers; the second receives the normal null success response. The public method's follow-up `GET_NOTEBOOK` returns the final title. Android is excluded: its public update path declares `replay_safe=False` and `_android/retry_policy.py` ceilings `MutateProject` to no replay. | `server_error_max_retries=1`; the public result is the refreshed `Notebook`. Exactly 2 byte-identical rename requests, the first commit is recorded, and the service fixture has one final notebook title. Recovery `get` returns `Stable`; HTTP handlers and admission settle. | Policy evidence: `_web/policy.py` registers `RENAME_NOTEBOOK` as a stable set operation; Android's policy manifest explicitly differs. Existing unit mutation tests cover request/decoding only. **New socket evidence.** I1, I2, I8. |
