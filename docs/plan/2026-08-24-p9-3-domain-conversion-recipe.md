...
The settings/suggestions PR is the pattern. A later domain PR converts every leaf the gate
table (`2026-08-24-p9-composite-gate-table.md` §7) lists as a codec row for that domain, one
domain per PR, and leaves composites, input-defaulting members and custom-row candidates as
handlers. Each step below is derivation, never a loosening of a guard.
...
- Remove the converted entries from `_HANDLER_NAMES` in `_web/registry.py`; nothing else there
  changes (the partition check `supported == handler names ∪ rows` and the 87/82/0 pins hold).
...
- Handler-name readers already accept rows: `test_semantic_p4_convergence_characterization.py`
  ...
  registry — update its literal row count (`== 4` today) per domain.
