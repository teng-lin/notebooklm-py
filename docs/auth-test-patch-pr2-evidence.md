...
| `tests/unit/test_auth_account_coverage.py::TestClearInBandLockFailure::test_lock_unavailable_is_swallowed` | best-effort clear leaves bytes untouched on unavailable lock, using a fresh `ProfileStore` | `src/notebooklm/_auth/profile_store.py` |
...
| `tests/unit/test_auth_stored_auth.py::test_inline_source_resolution_uses_real_environment_value` and `test_auth_json_env_owner_preserves_presence_and_raw_value` | real environment resolution produces an `InlineAuthSource` with captured account data; the canonical owner preserves unset, empty, and raw values | `src/notebooklm/_auth/tokens.py`, `src/notebooklm/_auth/paths.py`, `src/notebooklm/_auth/profile_document.py` |
