...
**Coverage update (2026-06, test-seam layout-coupling gap):** pattern 4's regex anchors on `notebooklm\._` — the private component must be the *first* path component — so the lint was structurally blind to **deep-leaf** private string targets like `patch("notebooklm.cli.session_cmd._sync_server_language_to_config")`, the exact form the #1481 CLI command-move post-mortem showed silently no-op'ing when a command body relocates. The lint now also flags (5) deep-leaf private string targets (at least one public component before the first `_`-prefixed one — deliberately **disjoint** from pattern 4, which keeps its drained-to-zero allowlist and stays-empty guard) and (6) private-*attribute-name* `patch.object(alias, "_private_attr")` patches, where the object reference is sound but the attribute string pins internal layout (only *full* dunder names — leading and trailing double underscore, e.g. `"__aenter__"` — are exempt; `"__private"`-style names are still flagged). Each new pattern carried its own **baselined** file-level allowlist (`_DEEP_LEAF_ALLOWLIST`, `_PATCH_OBJECT_PRIVATE_ATTR_ALLOWLIST`) holding the offenders measured at gate-landing time; unlike the original allowlist these started populated and drained opportunistically (stale-entry checks force removal of cleaned/deleted files, and no new entries are accepted). In the same change, the *remaining* layout coupling — string-target `patch("notebooklm…")` at **public** leaves, which no forbidden-pattern rule covers — was frozen by a per-file shrink-only population ratchet, `tests/_guardrails/test_string_patch_ratchet.py`: baselined files are pinned at their measured site counts, unbaselined files have a budget of zero, and ceilings only tighten.

**Drain complete (2026-08).** All three of those baselines are now **empty**: patterns (5) and (6) have no allowlisted files, and `STRING_PATCH_CEILINGS` shed all 52 originally-pinned files (768 sites). Every exemption structure in both gates is therefore at zero, and each gate is a *global* invariant — the string-patch ratchet in particular has become a flat ban, since the unbaselined-file budget of zero now applies to every file. Because a drained baseline is not self-locking (the tier gates skip allowlisted files, and a ceiling re-added at a file's current count reads as "not grown, not slack, not stale"), all three gained stays-empty guards mirroring the original `_ALLOWLIST == frozenset()` assertion: `test_baselined_allowlists_stay_empty` and `test_ceilings_stay_empty`. A contributor blocked by any of these gates must migrate the test to a sanctioned seam — re-baselining is no longer a green path. The two gates cross-reference each other in their remediation text so a violator is steered to the sanctioned seams (constructor injection via `tests/_fixtures/`, or `patch.object` on a *public* attribute of a locally-imported alias) rather than into the other gate's forbidden shape.
...
5. **Deep-leaf `unittest.mock` string-target patches into private internals** (added with the 2026-06 coverage update) — `patch("notebooklm.<public…>._private…")`, e.g. `patch("notebooklm.cli.session_cmd._sync_server_language_to_config")`. The same failure mode as (4) with the private component behind one or more public ones; disjoint from (4) by construction. Its file-level allowlist (`_DEEP_LEAF_ALLOWLIST`) landed baselined, has since drained to zero, and is now held there by a stays-empty guard.
6. **Private-attribute-name `patch.object` via a local alias** (added with the 2026-06 coverage update) — `patch.object(alias, "_private_attr", …)`. The object reference is real, but the attribute *name* string pins internal attribute layout and keeps "passing" against bag-of-attribute fakes after a rename. Only full dunder names (`"__aenter__"` etc.) are exempt — Python-protocol surface, not internal layout. Baselined in `_PATCH_OBJECT_PRIVATE_ATTR_ALLOWLIST`, which has since drained to zero and is now held there by a stays-empty guard; `patch.object` on a *public* attribute remains the sanctioned seam-alias form.
