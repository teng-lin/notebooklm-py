...
- **Pre-commit guard** (`tests/scripts/check_cassettes_clean.py`) re-scans every cassette in `tests/cassettes/` and rejects any cassette whose payload contains a known leak pattern. The guard runs as a standalone script — it is not invoked by `pytest`, so contributors must run it explicitly before pushing. (Memory note: this guard caught regressions that pytest missed during the tier-8 arc.)
...
The audit-ref-scrub cleanup (2026-05-17) added the `is_clean` filter helper so that *widening* a leak detector beyond its original provider allowlist would not re-flag the canonical placeholders (the placeholders contain provider-name substrings). The fix is captured in memory: *cassette scrubber idempotency — widening a leak detector beyond its provider allowlist will re-flag the canonical placeholder; fix via `SCRUB_PLACEHOLDERS` + `is_clean` filter, not regex negative-lookahead.*
