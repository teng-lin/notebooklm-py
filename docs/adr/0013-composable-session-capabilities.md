...
- **C-AC. Retire the legacy transport Adapter** — The backend-owned path is `WebExecutionRuntime -> RuntimeTransport -> RuntimePipeline -> RuntimeTransport.terminal -> Kernel.post`. Request construction lives in `_request_types.py`, typed call state in `_runtime/rpc_call_state.py`, transport exceptions and `Retry-After` parsing in `_transport_errors.py`, and size-capped streaming in `_streaming_post.py`.
