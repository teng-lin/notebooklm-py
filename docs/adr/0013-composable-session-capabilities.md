...
- **SHARED**: a capability used by ≥2 features today, justifying
  ...
  The concrete `CallSupervisor.operation_scope(...)` helper is used by sources
  upload plus artifact polling, but the named
...
- **C-X. `cookie_saver` compatibility adapter / `cookie_rotator` late-binding seam** — introduced by PR [#879](https://github.com/teng-lin/notebooklm-py/pull/879) (`76b301d`). The hooks are resolved through `_runtime/lifecycle.py` / `_runtime/init.py`, decoupling persistence from the core HTTP transport. A missing saver now selects the typed `ProfileStore` route unconditionally; only an explicit `cookie_saver=` uses the v0.x callback adapter. The rotator retains its late-bound default for keepalive integration.
...
- **C-AA. Drain-hook registration is owned by the call supervisor** — The standalone `DrainHookRegistration` Protocol from the Session/Kernel split was retired. `CallSupervisor.register_drain_hook(...)` owns the registration surface and the same generation counters own operation admission. Feature code that owns long-running async work — artifact polling in `_artifact/polling.py` — registers its close-time hook there, and `ClientLifecycle.close` fires the registered hooks. See ADR-0014.
