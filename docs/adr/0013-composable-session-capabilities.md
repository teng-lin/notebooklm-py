...
- **C-AC. Retire the authed-transport Adapter** — The middleware chain terminal now calls `Kernel.post` directly through `Session._authed_post_chain_terminal`. `_authed_transport.py` retains transport request types, exceptions, `Retry-After` parsing, and streaming helpers, while `_transport_errors.py` owns raw `Kernel.post` error mapping. This removed the last `_AuthedTransportHost` Protocol and the shallow `AuthedTransport` Adapter seam.
