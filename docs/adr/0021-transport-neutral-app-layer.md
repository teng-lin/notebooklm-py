...
- **Boundary.** `_app/` imports no transport framework (`click`, `rich`, `fastmcp`, `fastapi`, `starlette`, `uvicorn`), no adapter sibling (`notebooklm.cli`, `notebooklm.mcp`, `notebooklm.server`), no RPC runtime sibling (`notebooklm.rpc`), and no private `notebooklm._*` runtime sibling outside `_app` itself (enforced by `tests/_guardrails/test_app_boundary.py`). Rich/Click/FastMCP/FastAPI-coupled collaborators (consoles, prompters, spinners, importers, request/response objects) are **injected** as callables/Protocols. A neutral workflow may call a public `NotebookLMClient` namespace directly when that call contains no adapter resolution or presentation policy; resolver injection is required only where the adapter owns such policy. *Amended 2026-09-05:* the explicit adapter list now includes `notebooklm.mcp`, and direct public-client calls without adapter policy are permitted; the prior wording named `fastmcp` and required every resolver to be injected.
