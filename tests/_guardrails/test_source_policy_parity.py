"""Guardrail: the MCP and REST source adapters share source-policy definitions.

The batch/wait caps live in the transport-neutral ``_app`` core
(``_app.source_batch`` / ``_app.source_wait``). This gate forbids either
adapter from re-declaring the cap constants locally. Batch continuation is represented
by the public outcome contract and is deliberately not inferred from error categories.
"""
