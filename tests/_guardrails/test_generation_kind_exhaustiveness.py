...
sets, the per-kind RPC payload builders (``_web/codec/artifact_payloads.py``), and the
...
from notebooklm._web.codec import artifact_payloads
...
LOC: Mapping[str, str] = {
    ...
    "payloads module": _loc("_web/codec/artifact_payloads.py", r"^def build_audio_artifact_params"),
}
...
#: Per-kind RPC payload builders in ``_web/codec/artifact_payloads.py``. mind-map has TWO
...
def test_payload_builders_exist_per_kind() -> None:
    """Every kind has its RPC payload builder(s) in ``_web/codec/artifact_payloads.py``.
    ...
    assert missing == [], (
        f"{table} names builder(s) absent from src/notebooklm/_web/codec/artifact_payloads.py: "
        + ", ".join(missing)
    )
    ...
    _assert_parity(
        parity_failures(
            frozenset(claimed),
            actual,
            table="build_* functions in src/notebooklm/_web/codec/artifact_payloads.py",
