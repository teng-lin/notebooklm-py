...
UPLOAD_MODULE = PROJECT_ROOT / "src" / "notebooklm" / "_web" / "sources" / "upload.py"
    ...
    tree = ast.parse(
        "\n".join(
            [
                ...
                "from notebooklm._web.sources.upload import RpcCallback as Callback",
                "import notebooklm._web.sources.upload as RpcCallback",
            ]
        )
    )
    assign = tree.body[0]
