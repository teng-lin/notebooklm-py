...
class TestWorkflowTracerBullet:
    ...
    async def test_full_workflow(self, tmp_path: Path) -> None:
        ...
        async with NotebookLMClient(auth) as client:
            ...
            assert "#" in content, "downloaded report should contain at least one markdown heading"

            # ---------------------------------------------------------------
            # Teardown: delete the scratch notebook so the recording does not
            # leave orphan state in the user's account. Captured in the
            # cassette so replay also drives the delete RPC.
            # ---------------------------------------------------------------
