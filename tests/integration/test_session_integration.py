...
class TestClientInitialization:
    ...
    async def test_client_context_manager_closes(self, auth_tokens):
        async with NotebookLMClient(auth_tokens) as client:
            assert client._backend._kernel.http_client is not None  # client is open
        assert client._backend._kernel.http_client is None  # closed after exit
...
class TestGetHttpClient:
    ...
    via ``core._backend._kernel`` / ``client._backend._kernel``).
    """
    ...
    async def test_get_http_client_returns_client_when_initialized(self, auth_tokens):
        async with NotebookLMClient(auth_tokens) as client:
            http_client = client._backend._kernel.get_http_client()
...
class TestGetSourceIds:
    ...
    async def test_returns_empty_list_when_data_is_none(self, auth_tokens):
        async with NotebookLMClient(auth_tokens) as client:
            ...
            with patch.object(
                core._backend._runtime, "rpc_call", new_callable=AsyncMock, return_value=None
            ):
                ids = await notebooks.get_source_ids("nb_123")
    ...
    async def test_returns_empty_list_when_data_is_empty_list(self, auth_tokens):
        async with NotebookLMClient(auth_tokens) as client:
            ...
            with patch.object(
                core._backend._runtime, "rpc_call", new_callable=AsyncMock, return_value=[]
            ):
                ids = await notebooks.get_source_ids("nb_123")
...
class TestCrossDomainCookiePreservation:
    ...
    async def test_cookies_preserved_on_cross_domain_redirect(self, auth_tokens):
        ...
        async with NotebookLMClient(auth_tokens) as client:
            core = client
            http_client = core._backend._kernel.get_http_client()
    ...
    async def test_googleusercontent_cookies_not_reassigned(self, auth_tokens):
        ...
        async with NotebookLMClient(auth_tokens) as client:
            core = client
            http = core._backend._kernel.get_http_client()
...
class TestBuildUrlHL:
    ...
    ``core._backend._runtime.build_url(...)``.
