...
def test_note_backed_mind_map_service_signatures_are_frozen() -> None:
    """Freeze method signatures on NoteBackedMindMapService adapter."""
    assert list(inspect.signature(NoteBackedMindMapService.list_mind_maps).parameters) == [
        "self",
        "notebook_id",
    ]
    assert list(inspect.signature(NoteBackedMindMapService.extract_content).parameters) == [
        "self",
        "row",
    ]
    assert list(inspect.signature(NoteBackedMindMapService.delete_mind_map).parameters) == [
        "self",
        "notebook_id",
        "note_id",
    ]
    assert list(inspect.signature(NoteBackedMindMapService.rename_mind_map).parameters) == [
        "self",
        "notebook_id",
        "mind_map_id",
        "new_title",
...
async def test_note_service_fetch_note_rows_normalizes_containers_and_handles_schema_drift() -> (
    ...
    rows = await service.fetch_note_rows("nb-100")
    assert rows == [
        ["note-1", "Content 1"],
    ...
    assert await service.fetch_note_rows("nb-100") == [["note-flat-1", "Content"]]
    ...
    for empty_payload in [None, [], [[]], ["not-a-list"]]:
        ...
        assert await service.fetch_note_rows("nb-100") == []

    # 4. Truthy non-list schema drift raises DecodingError
    for drift_payload in ["string-drift", {"error": "unexpected"}, 42]:
        ...
        with pytest.raises(DecodingError) as exc_info:
            await service.fetch_note_rows("nb-100")
        assert exc_info.value.method_id == RPCMethod.GET_NOTES_AND_MIND_MAPS.value
...
def test_note_service_classify_row_exhaustiveness() -> None:
    ...
    assert service.classify_row(["n-del", None, 2, "extra"]) == NoteRowKind.DELETED

    # Mind map (JSON with 'children' or 'nodes')
    mm_children = ["n-mm1", '{"name": "Map", "children": [{"name": "Child"}]}']
    assert service.classify_row(mm_children) == NoteRowKind.MIND_MAP
    mm_nodes = ["n-mm2", '{"name": "Map", "nodes": []}']
    assert service.classify_row(mm_nodes) == NoteRowKind.MIND_MAP

    # Plain text note
    note_plain = ["n-txt", "This is plain markdown content"]
    assert service.classify_row(note_plain) == NoteRowKind.NOTE

    # Nested note format
    note_nested = ["n-nest", ["n-nest", "Nested body", None, None, "Title"]]
    assert service.classify_row(note_nested) == NoteRowKind.NOTE

    # None content (without deleted status 2) -> UNKNOWN
    assert service.classify_row(["n-none", None, 0]) == NoteRowKind.UNKNOWN
    assert service.classify_row(["n-none", None, 1]) == NoteRowKind.UNKNOWN

    # Invalid non-list or empty list -> UNKNOWN
    assert service.classify_row([]) == NoteRowKind.UNKNOWN
    assert service.classify_row(None) == NoteRowKind.UNKNOWN  # type: ignore[arg-type]
...
# ===========================================================================
# 4. NoteBackedMindMapService Adapter
# ===========================================================================
...
async def test_note_backed_mind_map_service_operations() -> None:
    ...
    # 1. list_mind_maps
    rows = await mind_maps.list_mind_maps("nb-100")
    assert len(rows) == 1
    assert NoteRow(rows[0]).id == "mm-1"
    assert mind_maps.extract_content(rows[0]) == mm_json
    ...
    await mind_maps.rename_mind_map("nb-100", "mm-1", "New Architecture Map")
    assert mock_session.rpc_executor.rpc_call.await_args_list[1] == call(
        RPCMethod.UPDATE_NOTE,
    ...
    with pytest.raises(MindMapNotFoundError) as exc_info:
        await mind_maps.rename_mind_map("nb-100", "mm-missing", "Title")
