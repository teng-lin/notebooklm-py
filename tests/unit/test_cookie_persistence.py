...
def test_client_core_exposes_cookie_persistence(tmp_path: Path) -> None:
    ...
    core._provider._persistence.loaded_cookie_snapshot = baseline

    assert isinstance(core._provider._persistence, CookiePersistence)
    assert core._provider._persistence.loaded_cookie_snapshot is baseline
    # The save lock is a stock ``threading.Lock`` owned by the collaborator.
    assert isinstance(core._provider._persistence.save_lock, type(threading.Lock()))
