...
``core._backend._reqid.next_reqid(...)``.
"""
...
async def test_next_reqid_concurrent_unique_and_monotonic() -> None:
    ...
    core = _make_core()
    step = 100000
    baseline = core._backend._reqid.value  # 100000

    results = await asyncio.gather(
        *[core._backend._reqid.next_reqid(step=step) for _ in range(100)]
    )
    ...
    assert core._backend._reqid.value == expected[-1]
...
async def test_next_reqid_concurrent_custom_step() -> None:
    ...
    baseline = core._backend._reqid.value

    results = await asyncio.gather(*[core._backend._reqid.next_reqid(step=step) for _ in range(50)])
