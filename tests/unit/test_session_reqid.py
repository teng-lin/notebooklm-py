...
``core._web_runtime.reqid.next_reqid(...)``. The ``_reqid_counter`` compat property +
...
``core._web_runtime.reqid.value`` directly and use ``core._web_runtime.reqid.set_value(...)`` to
seed a baseline.
"""
...
async def test_next_reqid_returns_post_increment_values() -> None:
    ...
    core = _make_core()
    assert core._web_runtime.reqid.value == 100000  # baseline

    first = await core._web_runtime.reqid.next_reqid()
    second = await core._web_runtime.reqid.next_reqid()
    third = await core._web_runtime.reqid.next_reqid()

    assert first == 200000
    assert second == 300000
    assert third == 400000
    # And the property reflects the final state.
    assert core._web_runtime.reqid.value == 400000
...
async def test_next_reqid_custom_step() -> None:
    ...
    core = _make_core()
    assert await core._web_runtime.reqid.next_reqid(step=1) == 100001
    assert await core._web_runtime.reqid.next_reqid(step=7) == 100008
    assert await core._web_runtime.reqid.next_reqid(step=1000) == 101008
...
async def test_next_reqid_does_not_warn() -> None:
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        await core._web_runtime.reqid.next_reqid()
        await core._web_runtime.reqid.next_reqid()
...
async def test_next_reqid_rejects_zero_step() -> None:
    ...
    with pytest.raises(ValueError, match="step must be positive"):
        await core._web_runtime.reqid.next_reqid(step=0)
    # Counter must not have moved.
    assert core._web_runtime.reqid.value == 100000
...
async def test_next_reqid_rejects_negative_step() -> None:
    ...
    core = _make_core()
    with pytest.raises(ValueError, match="step must be positive"):
        await core._web_runtime.reqid.next_reqid(step=-1)
    assert core._web_runtime.reqid.value == 100000
...
async def test_next_reqid_rejects_non_int_step() -> None:
    ...
    with pytest.raises(TypeError, match="step must be int"):
        await core._web_runtime.reqid.next_reqid(step="100")  # type: ignore[arg-type]
    assert core._web_runtime.reqid.value == 100000
...
async def test_next_reqid_rejects_bool_step() -> None:
    ...
    core = _make_core()
    with pytest.raises(TypeError, match="step must be int"):
        await core._web_runtime.reqid.next_reqid(step=True)  # type: ignore[arg-type]
    with pytest.raises(TypeError, match="step must be int"):
        await core._web_runtime.reqid.next_reqid(step=False)  # type: ignore[arg-type]
    assert core._web_runtime.reqid.value == 100000
