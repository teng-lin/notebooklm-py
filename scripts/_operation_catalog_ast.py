...
# The workflow body and the two report builders it shares with the other
# hoisted source-add workflows sit beside ``_source_service.py`` rather than in
# it: three hoisted workflows put that module over the ADR-0008 size budget.
REVIEWED_BACKEND_IMPORTS |= frozenset(
    {
        ("_source_add_failures.py", "_backend", "BackendContractError"),
        ("_source_add_failures.py", "_backend", "BackendError"),
        ("_source_add_failures.py", "_backend", "BackendErrorReason"),
        ("_source_add_failures.py", "_records", "SourceAddFailureRecord"),
